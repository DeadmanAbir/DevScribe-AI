...
function createFromUrl(url: string, config: Omit<YoutubeConfig, 'videoId'>): YoutubeConfig {
...
async function load(config: YoutubeConfig): Promise<Document[]> {
...
async function customLoader(url: string, language: string, addVideoInfo: boolean): Promise<Document[]> {
  const config = createFromUrl(url, { language: language, addVideoInfo: addVideoInfo });
  ...
  try {
    ...
    console.error(error);
    throw new Error(`Failed to get YouTube video transcription: ${(error as Error).message}`);
  }
}
